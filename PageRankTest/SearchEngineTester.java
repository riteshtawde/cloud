...
public class SearchEngineTester {
	...
	public static void searchKeyword(String keyword) throws Exception {
        ...
        for (KeyValue kv : indexRow.list()) {
			...
			Get row = new Get(kv.getQualifier());
			row.addColumn(Constants.CF_DETAILS_BYTES, Constants.QUAL_URI_BYTES);
			Result docIndexRow = dataTable.get(row);
			...
			row = new Get(kv.getQualifier());
			docIndexRow = prTable.get(row);
