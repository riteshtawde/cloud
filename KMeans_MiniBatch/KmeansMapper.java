...
public class KmeansMapper extends CollectiveMapper<String, String, Object, Object> {
	...
	miniBatchSize = configuration.getInt(KMeansConstants.NUM_ITERATONS, 100);
	...
	protected void mapCollective( KeyValReader reader, Context context) throws IOException, InterruptedException {
	 ...
	 private void computation(Table<DoubleArray> cenTable, Table<DoubleArray> previousCenTable,ArrayList<DoubleArray> dataPoints){
	  ...
	  private void runKmeans(List<String> fileNames, Configuration conf, Context context) throws IOException {
		  ...
		  for(int iter=0; iter < iteration; iter++){
			  ...
			  ArrayList<DoubleArray> samplePoints = loadMiniBatchData(dataPoints);
			  System.out.println("Iteraton No."+iter);
			  
			  //compute new partial centroid table using previousCenTable and data points
			  computation(cenTable, previousCenTable, samplePoints);
	  ...
	  private void calculateCentroids( Table<DoubleArray> cenTable, Table<DoubleArray> previousCenTable){
		  for( Partition<DoubleArray> partialCenTable: cenTable.getPartitions()){
			  Partition<DoubleArray> prevCenTablePart = previousCenTable.getPartition(partialCenTable.id());
			  double[] doubles = partialCenTable.get().get();
			  double[] prev = prevCenTablePart.get().get();

			  doubles[vectorSize] += prev[vectorSize];
			  for(int h = 0; h < vectorSize; h++){
				  doubles[h] = (prev[h] * prev[vectorSize] + doubles[h])/doubles[vectorSize];
			  }
	 ...
	 private void loadCentroids( Table<DoubleArray> cenTable, int vectorSize,  String cFileName, Configuration configuration) throws IOException{
	  ...
	  private ArrayList<DoubleArray> loadMiniBatchData(ArrayList<DoubleArray> dataPoints){
		int sampleSize = miniBatchSize/numMappers;
		int dataSize = dataPoints.size();
		ArrayList<Integer> randNumList = new ArrayList<Integer>();
		ArrayList<DoubleArray> samplePoints = new ArrayList<DoubleArray>();
		for(int i=0;i<sampleSize;i++){
			boolean done = false;
			Random rand = new Random();
			int randNum = rand.nextInt(dataSize);
			while(!done){
				if(!randNumList.contains(randNum)){
					samplePoints.add(dataPoints.get(randNum));
					randNumList.add(randNum);
					done = true;
				} else{
					randNum = rand.nextInt(dataSize);
				}
			}
		}
		System.out.println("sample points size :"+samplePoints.size());
		return samplePoints;
	  } 
}
